...
void loop() {
  // Checking if a face has been detected from the secondary arduino
  if (!faceDetected) {
    String newString;
    if (Serial.available()) {
  ...
  // Check the timer to see if it has run out, if not then update said timer.
    ...
    if (RFIDValid == "f") {
      ...
      mySerial.flush();
      delay(200); // Delay to ensure the secondary's response is finished before checking
    ...
    // If the RFID tag has been answered as correct, then check the question answer
      ...
      if (binaryAnswer.length() == 8) {
        ...
        // To be writted to the secondary arduino for checking
        for (int i = 0 ; i < 8; i++) {
        ...
        mySerial.flush();
        // A delay of 300 to allow the secondary arduino to recieve,process and send a response
        delay(300);
      }
      // If there is a message from the secondary
      if (mySerial.available()) {
        String result = recieveCommand();
        binaryAnswer = "";
        // If the secondary returned true then the "door" will be opened
        ...
        if (result == "t") {
          ...
          // If the secondary returns anything other then true
...
// Here we have a function used to recieve information from the secondary arduino
